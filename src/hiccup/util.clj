...
(defn maybe-escape-html
  "Change special characters into HTML character entities unless
  the input string was generated by a html macro."
  [text]
  (if (get *no-escape-strings* text)
    text
    (escape-html text)))

(defn without-escape-html
  "Turn some input into a string of HTML without escaping special
  characters."
  [text]
  (let [text (String. (to-str text))]
    (set! *no-escape-strings* (conj *no-escape-strings* text))
    text))
